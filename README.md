...
&emsp; The whole shell program operated in an infinite for loop which will run until the user
...
&emsp; After that, the cd or change directory cases were addressed in an if statement. If the first
...
&emsp; Following the cd cases, the input/output redirection will be addressed by checking if the
...
some error checks.\
&emsp; Finally, this code was completed by piping meaning the prompt had the potential to run
...
descriptors which used the dup() system call with respect to the standard read.1
